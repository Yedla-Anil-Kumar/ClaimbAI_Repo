...
# from agents.data_pipeline_agent import DataManagementMetrics


# def run_task(task):
#     """Worker function to run each task safely."""
#     try:
#         result = task["func"]()
#         with open(task["output"], "w", encoding="utf-8") as f:
#             json.dump(result, f, indent=2, ensure_ascii=False)
#         return f"{task['name']} evaluation complete. Results stored in {task['output']}"
...
#     metrics = DataManagementMetrics()

#     tasks = [
#         {
#             "name": "Schema Consistency",
#             "output": config["OUTPUT_PATH"],
#             "func": lambda: metrics.check_schema_consistency(
#                 metrics._load_json(config["BASELINE_PATH"]),
#                 metrics._load_json(config["TABLES_PATH"])
#             )
#         },
#         {
#             "name": "Data Freshness",
#             "output": config["FRESHNESS_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_data_freshness(metrics._load_json(config["FRESHNESS_INPUT_PATH"]))
#         },
#         {
#             "name": "Data Quality",
#             "output": config["QUALITY_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_data_quality(metrics._load_json(config["QUALITY_INPUT_PATH"]))
#         },
#         {
#             "name": "Governance Compliance",
#             "output": config["GOVERNANCE_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_governance_compliance(metrics._load_json(config["GOVERNANCE_INPUT_PATH"]))
#         },
#         {
#             "name": "Data Lineage",
#             "output": config["LINEAGE_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_data_lineage(metrics._load_json(config["LINEAGE_INPUT_PATH"]))
#         },
#         {
#             "name": "Metadata Coverage",
#             "output": config["METADATA_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_metadata_coverage(metrics._load_json(config["METADATA_INPUT_PATH"]))
#         },
#         {
#             "name": "Sensitive Tagging",
#             "output": config["TAGGING_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_sensitive_tagging(metrics._load_json(config["TAGGING_INPUT_PATH"]))
#         },
#         {
#             "name": "Duplication",
#             "output": config["DUPLICATION_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_duplication(metrics._load_json(config["DUPLICATION_INPUT_PATH"]))
#         },
#         {
#             "name": "Backup & Recovery",
#             "output": config["BACKUP_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_backup_recovery(metrics._load_json(config["BACKUP_INPUT_PATH"]))
#         },
#         {
#             "name": "Security Config",
#             "output": config["SECURITY_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_security_config(metrics._load_json(config["SECURITY_INPUT_PATH"]))
#         },
#     ]
...
# import yaml
# import os
# from agents.data_pipeline_agent import AnalyticsReadinessMetrics
...
#     # Instantiate metrics evaluator
#     metrics = AnalyticsReadinessMetrics()
...
#             "func": lambda: metrics.compute_pipeline_success_rate(
#                 metrics._load_json(config["PIPELINE_SUCCESS_INPUT_PATH"])
#             ),
#         },
#         {
#             "name": "Pipeline Latency & Throughput",
#             "output": config["PIPELINE_LATENCY_OUTPUT_PATH"],
#             "func": lambda: metrics.compute_pipeline_latency_throughput(
#                 metrics._load_json(config["PIPELINE_LATENCY_INPUT_PATH"])
#             ),
#         },
#         {
#             "name": "Resource Utilization Efficiency",
#             "output": config["RESOURCE_UTIL_OUTPUT_PATH"],
#             "func": lambda: metrics.evaluate_resource_utilization(
#                 metrics._load_json(config["RESOURCE_UTIL_INPUT_PATH"]),
#                 metrics._load_json(config["RESOURCE_UTIL_INPUT2_PATH"]),
#             ),
#         },
#         {
#             "name": "User Query Performance",
#             "output": config["QUERY_PERFORMANCE_OUTPUT_PATH"],
#             "func": lambda: metrics.assess_query_performance(
#                 metrics._load_json(config["QUERY_PERFORMANCE_INPUT_PATH"])
#             ),
#         },
#         {
#             "name": "Analytics Adoption",
#             "output": config["ANALYTICS_ADOPTION_OUTPUT_PATH"],
#             "func": lambda: metrics.compute_analytics_adoption(
#                 metrics._load_json(config["ANALYTICS_ADOPTION_INPUT_PATH"])
#             ),
#         },
#     ]

#     # Run tasks sequentially (can switch to parallel easily if required)
#     for task in tasks:
#         try:
#             result = task["func"]()
#             with open(task["output"], "w", encoding="utf-8") as f:
#                 json.dump(result, f, indent=2, ensure_ascii=False)
#             print(f"{task['name']} evaluation complete → {task['output']}")
#         except Exception as e:
    ...
    tasks.extend([
        ...
        },
        {
            ...
            "func": lambda: analytics_metrics.evaluate_resource_utilization(
                ...
                analytics_metrics._load_json(config["RESOURCE_UTIL_INPUT2_PATH"]),
            ),
        },
        {
            "name": "User Query Performance",
